...
if __name__ =="__main__":
    ...
    vgg_model.load_weights("y_eighth_weights.h5")
    ...
    #cap = cv.VideoCapture("dataset_video/3.mp4")
    cap = cv.VideoCapture("../../video_assets/cctv_별내동_01.mp4")
    ...
    while True:
        ...
        for i in range(len(boxes)):
            if i in indexes:
                x, y, w, h = boxes[i]
                if h > 40 and w > 40 and class_ids[i] == 2 and y > (vheight * 0.5):
                    cropimg = frame.copy()
                    copyimg = frame.copy()
                    ...
                    try:
                        ...
                        #imwrite('car_cropped_by_video/' + str(car_recog) + '/' + str(savecnt) + '.jpg', copyimg)
                        #savecnt += 1
                        #imwrite('car_cropped_by_video/' + str(car_recog) + '/' + str(savecnt) +'.jpg', cannimg)
                        #savecnt += 1
                        #print(car_recog)
                    except:
                        continue
                    ...
                    cv.putText(frame, car_recog, (x, y + 30), font, 2, color, 2)
                    #cv.putText(frame, label, (x, y + 20), font, 2, color, 2)
                elif class_ids[i] != 2:
                    label = str(classes[class_ids[i]])
                    color = colors[i]
                    cv.rectangle(frame, (x, y), (x + w, y + h), color, 2)
                    #cv.putText(frame, label, (x, y + 20), font, 2, color, 3)
                #cv.line(frame, (0,int(vheight*0.6)), (int(vwidth-1), int(vheight*0.6)), (0,0,0), 2)
        cv.imshow('', frame)
        framecnt += 1
        key = cv.waitKey(1)
        if key == 27:
